...
public class Arrays02 {
    ...
    public static void main(String[] args) {
        ...
        for(int w : numbers){
            minValue = Math.min(minValue, w);//Math.min() method compares two numbers and select the min one.
            maxValue = Math.max(maxValue, w);///Math.max() method compares two numbers and select the max one.
        }
